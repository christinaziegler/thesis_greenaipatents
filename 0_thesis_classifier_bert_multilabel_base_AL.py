...
def train_one_epoch(epoch, fold, model, model_name, data_loader, optimizer, criterion, device, accumulation_steps=4):
    model.train()
...
def cross_validate(model_name, df, abstract_column, oversample = True, minority_class = "SofAI", dropoutrate1 = 0.5, dropoutrate2 = 0.0, tokenizer=None, device='cpu', epochs=10, n_splits=5, batchsize = 16, optimizer_cls=AdamW, multilabel="binary"):
    ...
    for train_index, val_index in kf.split(np.zeros(len(df[minority_class].values)), df[minority_class].values):
        ...
        for epoch in range(epochs):
            train_loss, train_f1_0, train_f1_1 = train_one_epoch(epoch+1, fold, model, model_name, train_data_loader, optimizer, criterion, device, accumulation_steps = 4)
    ...
    return avg_loss, avg_f1_micro, avg_f1_per_category, avg_accuracy, f1_range, model
...
def select_samples_by_uncertainty(prediction_probs, top_k=10):
    entropies = -np.sum(prediction_probs * np.log(prediction_probs + 1e-5), axis=1)  # add a small constant to avoid log(0)
    uncertain_indices = np.argsort(entropies)[-top_k:]
    return uncertain_indices

def generate_filename(model_name, abstract_column, optimizer_name, labels, batchsize, dropoutrate1, dropoutrate2, suffix=''):
    return f"{labels}_{model_name}_{abstract_column}_{optimizer_name}_batch{batchsize}_dropout{dropoutrate1}and{dropoutrate2}_{suffix}"


# In[36]:

df = training_data
unseen_df = unseen_data
...
for modelname in ["BERT"]:
    for abstract_column in ["text_descr300"]:
        for dropoutrate1 in [0.5]:
            for dropoutrate2 in [0.0]:
                for batchsize in [16]:
                    base_filename = generate_filename(model_name=modelname, abstract_column=abstract_column, optimizer_name=optimizer_cls.__name__, labels=multilabel, 
                                                    batchsize=batchsize, 
                    ...
                    avg_loss, avg_f1_micro, avg_f1_per_category, avg_accuracy, f1_range, model = cross_validate(model_name=modelname, df=df, abstract_column=abstract_column, 
                                                                    minority_class="SofAI", 
                                                                    dropoutrate1 = dropoutrate1, dropoutrate2=dropoutrate2, 
                                                                    tokenizer=tokenizer, device=device, epochs=3, 
                                                                    batchsize=batchsize, n_splits=3, optimizer_cls=optimizer_cls, 
                                                                    multilabel=multilabel)
                    unseen_dataset = PatentTestDataset(unseen_df, tokenizer=tokenizer, abstract_column=abstract_column)
                    unseen_data_loader = DataLoader(unseen_dataset, batch_size=batchsize, shuffle=True)
                    predictions, prediction_probs = predict_with_uncertainty(model=model, data_loader= unseen_data_loader, device=device)
                    uncertain_indices = select_samples_by_uncertainty(prediction_probs, top_k = 50)
                    print([unseen_df[i].publication_number_EPO for i in uncertain_indices])
