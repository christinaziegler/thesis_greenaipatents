...
class BertClassifier(nn.Module):
    def __init__(self, model_name = 'bert-base-uncased', num_labels = 2, freeze_layers=0, dropout_rate1=0.5, dropout_rate2=0.0):
        ...
        if model_name == 'allenai/longformer-base-4096':  
            config = LongformerConfig.from_pretrained(model_name, num_labels=num_labels)
            self.bert = LongformerForMultiLabelSequenceClassification(config)
        else:
    ...
    # Define your forward pass using checkpoint
        return checkpoint(self._forward, input_ids, attention_mask)
...
def cross_validate(model_name, df, loss_func_name, abstract_column, minority_class = "SofAI", dropoutrate1 = 0.5, dropoutrate2 = 0.0, tokenizer=None, device='cpu', epochs=10, n_splits=5, batchsize = 16, optimizer_cls=AdamW, save=False, multilabel="binary"):
        ...
        for dropoutrate1 in [0.5]:
            for dropoutrate2 in [0.2]:
                for loss_func_name in ["CBloss-ntr", "DBloss", "BCE"]:
                    base_filename = generate_filename(model_name=modelname, abstract_column=abstract_column, optimizer_name=optimizer_name, loss_func_name=loss_func_name, labels=multilabel, 
                                                        batchsize=batchsize, 
                    ...
                    with open(f'performance_dict_imbalancedloss_{loss_func_name}_{multilabel}_base_{timestamp}.json', 'w') as f:
