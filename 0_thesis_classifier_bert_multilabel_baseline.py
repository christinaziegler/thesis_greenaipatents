...
def train_one_epoch(epoch, fold, model, model_name, data_loader, optimizer, criterion, device, accumulation_steps=4):
    ...
    plt.show()
    plt.savefig(f"{multilabel}_{model_name}_{abstract_column}_{optimizer_name}_batch{batchsize}_dropout{dropoutrate1}and{dropoutrate2}_confusionmatrix_fold{fold}_epoch{epoch}.png")
...
def cross_validate(model_name, df, abstract_column, oversample = True, minority_class = "SofAI", dropoutrate1 = 0.5, dropoutrate2 = 0.0, tokenizer=None, device='cpu', epochs=10, n_splits=5, batchsize = 16, optimizer_cls=AdamW, multilabel="binary"):
    ...
    cm_display.plot(cmap='Blues')
    plt.show()
    plt.savefig(f"{multilabel}_{model_name}_{abstract_column}_{optimizer_name}_batch{batchsize}_dropout{dropoutrate1}and{dropoutrate2}_confusionmatrix_full.png")
...
for modelname in ["BERT", "SciBERT", "PatentsBerta"]:
    for abstract_column in ["text_descr300", "text_abstr"]:
        for dropoutrate1 in [0.2, 0.5]:
            for dropoutrate2 in [0.0, 0.2]:
                for batchsize in [16, 32]:
                    ...
                    with open(f'performance_dict_tests_{multilabel}_base_{timestamp}.json', 'w') as f:
