...
class PatentDataset(Dataset):
    def __init__(self, df, abstract_column, other_column, cpc_column="cpc_codes_EPO",
        ...
        self.cat_features = df.drop(columns=[abstract_column, other_column] + label_columns).values.tolist()
...
def train_one_epoch(epoch, fold, model, model_name, loss_func, num_labels, data_loader, optimizer, device, accumulation_steps=4):
    ...
    plt.show()
    plt.savefig(f"finalmodel_{multilabel}_{model_name}_multimodal_{optimizer_name}_batch{batchsize}_dropout{dropoutrate1}and{dropoutrate2}_confusionmatrix_fold{fold}_epoch{epoch}.png")
...
def cross_validate(model_name, df, loss_func_name, abstract_column, oversample = True, minority_class = "SofAI", dropoutrate1 = 0.5, dropoutrate2 = 0.0, tokenizer=None, device='cpu', epochs=10, n_splits=5, batchsize = 16, optimizer_cls=AdamW, multilabel="binary"):
    ...
    cm_display.plot(cmap='Blues')
    plt.show()
    plt.savefig(f"finalmodel_{multilabel}_{model_name}_multimodal_{optimizer_name}_batch{batchsize}_dropout{dropoutrate1}and{dropoutrate2}_confusionmatrix_full_{timestamp}.png")
                ...
                for batchsize in [32]:
                    for loss_func_name in ["CBloss-ntr"]:
                        ...
                        avg_loss, avg_f1_macro, avg_f1_per_category, avg_accuracy, f1_range, model = cross_validate(model_name=modelname, df=df, loss_func_name =  loss_func_name,
                                                                        ...
                                                                        multilabel=multilabel)
                        test_dataset = PatentTestDataset(test_df, tokenizer=tokenizer, abstract_column=abstract_column, other_column=other_column, cpc_column = cpc_column)
                        ...
                        for idx, label in enumerate(label_columns):
                            test_df[f'predicted_{label}'] = predicted_labels[:, idx]

                        
                        # Collect all true and predicted labels
                        true_labels = test_df[label_columns].values
                        predicted_labels = test_df[[f'predicted_{label}' for label in label_columns]].values
                        ...
                        performance_dict[base_filename] = {
                                                            ...
                                                            "Average Test precision": average_precision,
                                                            "Average Test recall": average_recall,
                                                            "Average Test F1": average_f1
                                                        }

                        # save performance_dict to a file with timestamp in the filename
                        with open(f'performance_dict_final_model_{timestamp}.json', 'w') as f:
                            json.dump(performance_dict, f)
                        # Compute and plot the confusion matrix
                        cm = compute_combination_confusion_matrix(true_labels, predicted_labels)
                        labels = ['[0, 0]', '[0, 1]', '[1, 0]', '[1, 1]']
                        cm_display = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=labels)
                        cm_display.plot(cmap='Blues')
                        ...
                        plt.savefig(f"finalmodel_testset_confusion_matrix_{timestamp}.png")
