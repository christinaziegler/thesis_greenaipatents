...
# Load unseen data
unseen_data = pd.read_csv("unseen_data_new.csv")

# Extract CPC codes 
pattern = r"'(.*?)'"
training_data["cpc_codes_EPO"] = training_data["cpc_codes_EPO"].apply(lambda x: set(re.findall(pattern, x)))
unseen_data["cpc_codes_EPO"] = unseen_data["cpc_codes_EPO"].apply(lambda x: set(re.findall(pattern, x)))

# Combine all unique CPC codes from training and unseen data
all_cpc_codes = set.union(*training_data['cpc_codes_EPO'], *unseen_data['cpc_codes_EPO'])

# Fit the MultiLabelBinarizer on the combined CPC codes
mlb = MultiLabelBinarizer(classes=list(all_cpc_codes))
mlb.fit(list(all_cpc_codes))  # Fit with all unique CPC codes

# Transform the training data using the fitted binarizer
cpc_encoded_train = mlb.transform(training_data['cpc_codes_EPO'])
cpc_encoded_df_train = pd.DataFrame(cpc_encoded_train, columns=mlb.classes_)

# Concatenate the encoded CPC codes with the original training dataframe
training_data = pd.concat([training_data.drop(columns=['cpc_codes_EPO', 'Unnamed: 0', 'Sus of AI', 'AI for Sus',
       'abstract', 'title', 'description', 'claims',
       'text_descr', 'text_descr_claims',
       'text_descr3000_claims', 'text_claims']), cpc_encoded_df_train], axis=1)

# Transform the unseen data using the same binarizer
print(unseen_data.title.isnull().sum())
print((unseen_data['cpc_codes_EPO'] == '').sum())
print(unseen_data.cpc_codes_EPO.isnull().sum())
cpc_encoded_unseen = mlb.transform(unseen_data['cpc_codes_EPO'])
cpc_encoded_df_unseen = pd.DataFrame(cpc_encoded_unseen, columns=mlb.classes_)
publication_numbers_unseen = unseen_data['publication_number_EPO']
# Concatenate the encoded CPC codes with the original unseen dataframe
unseen_data = pd.concat([unseen_data.drop(columns=['cpc_codes_EPO', 'Unnamed: 0', 
       'abstract', 'title', 'description', 'claims',
       'text_descr', 'text_descr_claims',
       'text_descr3000_claims', 'text_claims']), cpc_encoded_df_unseen], axis=1)

# Verify the dimensions are consistent
print(f"Training data shape: {training_data.shape}")
print(f"Unseen data shape: {unseen_data.shape}")
...
class PatentDataset(Dataset):
    def __init__(self, df, abstract_column, other_column, cpc_column="cpc_codes_EPO", publication_numbers = "publication_number_EPO",
        ...
        self.cat_features = df.drop(columns=[abstract_column, other_column, publication_numbers] + label_columns).values.tolist()
...
class BertClassifier(nn.Module):
    def __init__(self, num_cat_features, model_name = 'bert-base-uncased', num_labels = 2, freeze_layers=0, dropout_rate1=0.5, dropout_rate2=0.0):
    ...
    def forward(self, input_ids, attention_mask, nontext_features):
        ...
        # Concatenate BERT output with non-text features
...
def train_one_epoch(epoch, fold, model, model_name, loss_func, num_labels, data_loader, optimizer, device, accumulation_steps=4):
    ...
    labels = ['Both', 'AI for Sustainability', 'Sustainability of AI', 'Both']
...
def cross_validate(model_name, df, loss_func_name, abstract_column, oversample = True, minority_class = "SofAI", dropoutrate1 = 0.5, dropoutrate2 = 0.0, tokenizer=None, device='cpu', epochs=10, n_splits=5, batchsize = 16, optimizer_cls=AdamW, multilabel="binary"):
    if abstract_column == "text_descr300":
        other_column = "text_abstr"
    elif abstract_column == "text_abstr":
        other_column = "text_descr300"
    dataset = PatentDataset(df, abstract_column=abstract_column, other_column=other_column, cpc_column = cpc_column, tokenizer=tokenizer)
    ...
    return avg_loss, avg_f1_macro, avg_f1_per_category, avg_accuracy, f1_range, model

       
    
class PatentTestDataset(Dataset):
    def __init__(self, df, abstract_column,other_column, cpc_column="cpc_codes_EPO", publication_numbers = "publication_number_EPO",
                 tokenizer=None, cat_encoder=None): 
            self.df = df
            self.abstracts = df[abstract_column].tolist()  # Directly use the numerical labels
            self.tokenizer = tokenizer
            self.cat_features = df.drop(columns=[abstract_column, other_column, publication_numbers]).values.tolist()
            self.num_cat_features = len(self.cat_features[0]) if self.cat_features else 0
...
def predict(model, model_name, data_loader, device):
    ...
    with torch.no_grad():
        for i, data in enumerate(tqdm(data_loader, desc="Predicting")):
...
for modelname in ["SciBERT"]:
    for abstract_column in ["text_descr300"]:
        for dropoutrate1 in [0.2]:
            for dropoutrate2 in [0.2]:
                for batchsize in [32]:
                    for loss_func_name in ["CBloss-ntr"]:
                        ...
                        if abstract_column == "text_descr300":
                            ...
                            other_column = "text_descr300"
                        train_dataset = PatentDataset(df, tokenizer=tokenizer, abstract_column=abstract_column, other_column=other_column, cpc_column = cpc_column)
                        train_data_loader = DataLoader(train_dataset, batch_size=batchsize, shuffle=True)
                        avg_loss, avg_f1_macro, avg_f1_per_category, avg_accuracy, f1_range, model = cross_validate(model_name=modelname, df=df, loss_func_name =  loss_func_name,
                                                                        ...
                                                                        multilabel=multilabel)
                        test_dataset = PatentTestDataset(unseen_data, tokenizer=tokenizer, abstract_column=abstract_column, other_column=other_column, cpc_column = cpc_column)
                        test_data_loader = DataLoader(test_dataset, batch_size=batchsize)
                        
                        predicted_labels = predict(model, modelname, test_data_loader, device=device)
                        print(f"Number of predictions: {len(predicted_labels)}")
                        print(f"Expected number of predictions: {len(unseen_data)}")
                        # Ensure predicted_labels is a NumPy array
                        predicted_labels = np.array(predicted_labels)
                        

                        # Adding predictions to the test dataframe
                        # Adjust the following lines as per your multi-label format
                        for idx, label in enumerate(label_columns):
                            unseen_data[f'predicted_{label}'] = predicted_labels[:, idx]
                        

                        label_columns = ["predicted_SofAI", "predicted_AIforS"]  # Update this list with all your predicted label columns
                        publication_number_column = "publication_number_EPO"
                        abstract_column = "text_descr300"

                        # Create a list of columns to keep
                        columns_to_keep = label_columns + [publication_number_column, abstract_column]

                        # Filter the DataFrame
                        unseen_data_filtered = unseen_data[columns_to_keep]
                        print(len(unseen_data_filtered))
                        print(unseen_data_filtered.head())  # Print the first few rows
                        print(unseen_data_filtered.shape)
                        #unseen_data['publication_number_EPO'] = publication_numbers_unseen
                        print(f"Validation Accuracy: {avg_accuracy:.2%}")
                        print(f"Validation F1-score: {avg_f1_macro:.2%}")

                        # Update performance_dict with the overall metrics

                        performance_dict[base_filename] = {
                                                            "Average Val Loss": avg_loss,
                                                            "Average Val macro-F1": avg_f1_macro,
                                                            "Val F1 per Category 0": avg_f1_per_category[0],
                                                            "Val F1 per Category 1": avg_f1_per_category[1],
                                                            "Val Average Accuracy": avg_accuracy,
                                                            "Val F1 Range": f1_range
                                                        }
                        timestamp = time.strftime("%Y%m%d-%H%M%S")
                        unseen_data_filtered.to_csv(f"unseen_data_predictions_{timestamp}.csv", index=False)
                        # save performance_dict to a file with timestamp in the filename
                        
                        with open(f'performance_dict_finalmodel_predictions_{timestamp}.json', 'w') as f:
